...
ears <- subset(data_2013, ear == 'E')
earsD10 <- subset(ears, (siteid == 'STER' | siteid == "CPER"))
earsD01<- subset(ears, (siteid == 'HARV'))
earsD03 <- subset(ears, (siteid == 'OSBS' | siteid == 'DSNY'))

sqlStatement <- paste("SELECT taxonid, COUNT(taxonid) FROM mammals_neon.capture_data_2013 GROUP BY taxonid, siteid, ear HAVING siteid = 'OSBS')", sep="")
earsbysp <- dbGetQuery(conn, sqlStatement)
